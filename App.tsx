...
import { generatePosterImage, translateDishToEnglish } from './services/geminiService';
import LayoutPreview from './components/LayoutPreview';
import DishInputForm from './components/DishInputForm';
import ResultArea from './components/ResultArea';
import { Settings, Archive, Palette, ChefHat, Image as ImageIcon, Zap, AlertTriangle, RefreshCw, Moon, Sun, Key } from 'lucide-react';
...
export default function App() {
  ...
  const handleGenerate = async () => {
    // If Nano Banana mode, check API Key first
    if (mode === AppMode.NANO_BANANA && !hasApiKey) {
        await handleSelectKey();
        // Return here, let user click again after selection to avoid confusion or failed calls
        return; 
    }
    ...
    if (mode === AppMode.NANO_BANANA) {
      try {
        ...
        // Improved error stringification for checking type
        const errorMsg = e.message || e.toString() || JSON.stringify(e);
        // If permission denied, reset key state
  ...
  return (
    <div className="flex h-screen w-full bg-gray-100 dark:bg-gray-900 font-sans transition-colors duration-200">
      ...
      {/* LEFT SIDEBAR: Config */}
      <div className="w-80 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col z-10 shadow-lg">
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-8">
                ...
                <div className="space-y-2">
                    ...
                    <div className="mt-2">
                        <input 
                            ...
                            className="w-full text-xs p-2 border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-700 text-gray-800 dark:text-gray-200 rounded focus:border-brand-500 outline-none"
                        />
                    </div>
                </div>
            </section>
            ...
            {/* Nano Config */}
            {mode === AppMode.NANO_BANANA && (
                <section className="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg border border-gray-100 dark:border-gray-700">
                    ...
                    <div className="flex items-center justify-between text-xs mb-2">
                        ...
                        <select 
                            ...
                            className="bg-white dark:bg-gray-600 border border-gray-200 dark:border-gray-500 text-gray-700 dark:text-gray-200 rounded p-1"
                        >
        ...
        <div className="p-4 border-t border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
             <button 
                ...
                className="w-full flex items-center justify-center space-x-2 py-2 bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 rounded text-xs font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-600 disabled:opacity-50"
             >
      ...
      {/* MIDDLE: Input Form */}
      <div className="flex-1 overflow-y-auto bg-gray-50/50 dark:bg-gray-900 p-8 min-w-[400px]">
        <div className="max-w-3xl mx-auto">
            <div className="mb-6 flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-bold text-gray-800 dark:text-white">Dish Input</h2>
                    <p className="text-gray-500 dark:text-gray-400 text-sm mt-1">Configure {layout === LayoutType.LAYOUT_4 ? '6 dishes' : (layout === LayoutType.LAYOUT_3 ? '2 dishes' : '1 dish')} for generation.</p>
                </div>
                <div className="bg-white dark:bg-gray-800 px-4 py-1.5 rounded-full border border-gray-200 dark:border-gray-700 text-xs font-mono text-gray-500 dark:text-gray-400 shadow-sm">
                    {results.length} items generated
                </div>
            </div>

            {/* Updated to use handleDishUpdate for bulk updates */}
            <DishInputForm layout={layout} dishes={dishes} onUpdate={handleDishUpdate} />

            <div className="mt-8">
                <button
                    onClick={handleGenerate}
                    disabled={isGenerating || (mode === AppMode.NANO_BANANA && !hasApiKey && !dishes.some(d => !d.name))}
                    className={`w-full py-4 rounded-xl text-white font-bold text-lg shadow-xl flex items-center justify-center space-x-3 transition-all transform hover:-translate-y-1 hover:shadow-2xl disabled:opacity-50 disabled:cursor-not-allowed ${mode === AppMode.NANO_BANANA ? 'bg-gradient-to-r from-amber-500 to-orange-600 dark:from-amber-600 dark:to-orange-700' : 'bg-gray-900 dark:bg-gray-700'}`}
                >
                    {isGenerating ? (
                        <>
                        ...
                        <>
                            {mode === AppMode.NANO_BANANA ? <ImageIcon className="w-6 h-6" /> : <ChefHat className="w-6 h-6" />}
                            <span>
                                {mode === AppMode.NANO_BANANA && !hasApiKey 
                                    ? 'Select API Key to Generate' 
                                    : `Generate ${mode === AppMode.NANO_BANANA ? 'Poster' : 'Prompts'}`
                                }
                            </span>
                        </>
                    )}
      ...
      {/* RIGHT: Results */}
      <div className="w-96 bg-white dark:bg-gray-800 border-l border-gray-200 dark:border-gray-700 flex flex-col z-10 shadow-lg">
